...
Check that `first-use-onboarding-agent`, the `first-use-onboarding` skill, and `/jm-adk:first-use` exist.
...
Use `runtime-routing-agent`. Mark the capability as validation pending and fall back to a Markdown-first local workflow.
