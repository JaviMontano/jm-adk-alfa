...
- [CODE] `python3 -B scripts/validate-skill-dod.py --skill admin-dashboards` returned `dod=pass errors=0`.
...
- [CODE] `skills/admin-dashboards/evals/evals.json` now uses the `cases` contract and includes `assets`, `deterministic_scripts`, and `quality_criteria` in `expected_checks` coverage.
