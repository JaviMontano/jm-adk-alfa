#!/usr/bin/env bash
# Deterministic script contract check for firma-pdf-legal.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
validator="$script_dir/signature_packet_lint.py"
...
echo "firma-pdf-legal check passed: valid=$valid_count invalid=$invalid_count"
