...
6. Valida el plan con `scripts/validate_orchestration_plan.py`.

Detalle de construcción y checklist en `SKILL.md`. Los assets determinísticos viven en `assets/`.

## Offline Validation

```bash
bash skills/subagent-orchestration/scripts/check.sh
python3 -B scripts/validate-skill-dod.py --skill subagent-orchestration
python3 -B scripts/validate-skill-scripts.py --strict --run-checks --skill subagent-orchestration
```

El check local prueba planes válidos, planes bloqueados y planes inválidos.
