...
VALIDATOR="${SCRIPT_DIR}/validate_ai_architecture_audit_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/ai-architecture-audit-invalid.out 2>/tmp/ai-architecture-audit-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "ai-architecture-audit fixture validation passed"
fi
