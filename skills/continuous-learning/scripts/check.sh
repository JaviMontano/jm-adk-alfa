...
VALIDATOR="${SCRIPT_DIR}/validate_continuous_learning_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/continuous-learning-invalid.out 2>/tmp/continuous-learning-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "continuous-learning fixture validation passed"
fi
