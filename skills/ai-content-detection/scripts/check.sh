...
VALIDATOR="${SCRIPT_DIR}/validate_ai_content_detection_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  out_file="${TMP_ROOT}/ai-content-detection-invalid-$(basename "${report}").out"
  err_file="${TMP_ROOT}/ai-content-detection-invalid-$(basename "${report}").err"
...
if [[ "${status}" -eq 0 ]]; then
  echo "ai-content-detection fixture validation passed"
fi
