...
VALIDATOR="${SCRIPT_DIR}/validate_ai_code_review_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  out_file="${TMP_ROOT}/ai-code-review-invalid-$(basename "${report}").out"
  err_file="${TMP_ROOT}/ai-code-review-invalid-$(basename "${report}").err"
...
if [[ "${status}" -eq 0 ]]; then
  echo "ai-code-review fixture validation passed"
fi
