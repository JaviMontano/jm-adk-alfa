...
VALIDATOR="${SCRIPT_DIR}/validate_environment_detection_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/environment-detection-invalid.out 2>/tmp/environment-detection-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "environment-detection fixture validation passed"
fi
