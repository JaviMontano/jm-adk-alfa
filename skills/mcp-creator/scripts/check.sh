...
VALIDATOR="${SCRIPT_DIR}/validate_mcp_config_plan.py"
...
for plan in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${plan}" >/tmp/mcp-creator-invalid.out 2>/tmp/mcp-creator-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "mcp-creator check passed: valid=${valid_count} invalid=${invalid_count}"
fi
