...
version: 0.1.0
description: "Simula entrevista una pregunta a la vez con feedback en 3 rubricas separadas: sustancia, ingles, presencia."
owner: "JM Labs"
triggers:
...
`scripts/score_rubric.py --input <json>` scores an answer on three SEPARATE rubrics (substance, English, presence) + question bank in `scripts/question_bank.json`. Exit 1 if a dimension is below floor. Deterministic checks: `scripts/check.sh` + fixtures.
