...
version: 0.1.0
description: "Recomputa y valida una liquidacion laboral colombiana (cesantias, intereses, prima, vacaciones) y marca paz y salvo."
owner: "JM Labs"
triggers:
  - validar-liquidacion
  - liquidacion-co
...
`scripts/validate_liquidacion.py --input <json>` recomputes Colombian severance (cesantías, intereses 12%, prima, vacaciones), checks sum/neto, flags paz-y-salvo, lists what to ask. Exit 1 on deviation. Deterministic checks: `scripts/check.sh` + fixtures.
