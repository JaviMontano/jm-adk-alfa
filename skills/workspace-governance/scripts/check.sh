...
VALIDATOR="${SCRIPT_DIR}/validate_workspace_governance_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/workspace-governance-invalid.out 2>/tmp/workspace-governance-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "workspace-governance fixture validation passed"
fi
