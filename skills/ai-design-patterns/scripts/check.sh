...
VALIDATOR="${SCRIPT_DIR}/validate_ai_design_patterns_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/ai-design-patterns-invalid.out 2>/tmp/ai-design-patterns-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "ai-design-patterns fixture validation passed"
fi
