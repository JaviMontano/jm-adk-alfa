...
VALIDATOR="${SCRIPT_DIR}/validate_alerting_strategy.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/alerting-strategy-invalid.out 2>/tmp/alerting-strategy-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "alerting-strategy fixture validation passed"
fi
