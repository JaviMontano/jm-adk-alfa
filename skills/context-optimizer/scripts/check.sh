...
VALIDATOR="${SCRIPT_DIR}/validate_context_optimizer_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/context-optimizer-invalid.out 2>/tmp/context-optimizer-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "context-optimizer fixture validation passed"
fi
