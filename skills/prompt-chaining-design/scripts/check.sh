...
SKILL_DIR="$ROOT/skills/prompt-chaining-design"
VALIDATOR="$SKILL_DIR/scripts/validate_prompt_chaining_design.py"
...
validator_path = skill_dir / "scripts" / "validate_prompt_chaining_design.py"
spec = importlib.util.spec_from_file_location("prompt_chain_validator", validator_path)
...
for case in mutations:
    ...
    if not module.validate(mutated):
...
printf 'prompt-chaining-design check passed: valid=%s invalid=%s\n' "$valid_count" "$invalid_count"
