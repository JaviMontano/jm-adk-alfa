#!/usr/bin/env bash
# Deterministic script contract check for negociacion-oferta.

set -euo pipefail

script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
validator="$script_dir/score_oferta.py"
...
for fixture in "$script_dir"/fixtures/blocked-*.json; do
  if python3 "$validator" --input "$fixture" >/dev/null 2>&1; then
    echo "ERROR: expected blocked fixture to have no passing offer: $fixture" >&2
    exit 1
  fi
...
echo "negociacion-oferta check passed: valid=$valid_count blocked=$blocked_count invalid=$invalid_count"
