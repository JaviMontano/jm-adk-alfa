...
SCRIPT="$SKILL_DIR/scripts/validate_code_review_report.py"
TAXONOMY="$SKILL_DIR/assets/review-taxonomy.json"
...
python3 -m json.tool "$SKILL_DIR/scripts/fixtures/invalid-approval-with-blocker.json" >/dev/null
python3 -m json.tool "$SKILL_DIR/scripts/fixtures/invalid-untagged-report.json" >/dev/null
...
python3 -B "$SCRIPT" \
  ...
  --evidence "$EVIDENCE" \
  --boundary "$BOUNDARY" \
  --report "$SKILL_DIR/scripts/fixtures/valid-blocking-report.json" \
...
python3 -B "$SCRIPT" \
  ...
  --evidence "$EVIDENCE" \
  --boundary "$BOUNDARY" \
  --report "$SKILL_DIR/scripts/fixtures/valid-clean-report.json" \
  --expect pass >/dev/null
...
python3 -B "$SCRIPT" \
  ...
  --evidence "$EVIDENCE" \
  --boundary "$BOUNDARY" \
  --report "$SKILL_DIR/scripts/fixtures/invalid-approval-with-blocker.json" \
...
python3 -B "$SCRIPT" \
  ...
  --evidence "$EVIDENCE" \
  --boundary "$BOUNDARY" \
  --report "$SKILL_DIR/scripts/fixtures/invalid-untagged-report.json" \
  --expect fail >/dev/null

echo "OK: code-review reports validated deterministically"
