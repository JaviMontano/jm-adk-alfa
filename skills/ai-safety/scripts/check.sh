...
VALIDATOR="${SCRIPT_DIR}/validate_ai_safety_report.py"
...
for report in "${FIXTURE_DIR}"/invalid-*.json; do
  if python3 -B "${VALIDATOR}" "${report}" >/tmp/ai-safety-invalid.out 2>/tmp/ai-safety-invalid.err; then
...
if [[ "${status}" -eq 0 ]]; then
  echo "ai-safety fixture validation passed"
fi
